 ...
 * @returns {Boolean}
 */
export function date_is_dst(date) {
  const base_offset =
 ...
 */
function lookup_key() {
  const january_offset = get_january_offset(),
 ...
 * @returns Object
 */
export function determine_timezone() {
  const key = lookup_key();
  return new TimeZone(olson.timezones[key]);
}
 ...
 *
 * @constructor
 * @param {Array} tz_info - An array containing configuration parameters from module.olson.timezones.
 */
export function TimeZone(tz_info) {
  this.timezone_name = tz_info[1];
  this.uses_dst = tz_info[2];
  this.utc_offset = tz_info[0];
