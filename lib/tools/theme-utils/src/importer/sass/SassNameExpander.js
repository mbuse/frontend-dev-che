/**
 * Adapted from [Eyeglass](https://github.com/sass-eyeglass/eyeglass)
 **/

const path = require("path");
const validExtensions = [".scss", ".sass", ".css"];
const partialPrefix = "_";

class SassNameExpander {
  /**
  ...
  addLocation(location) {
    ...
    if (location && location !== "stdin") {
      ...
      const fullLocation = path.join(location, this.uri);
...
function withFileVariants(name, callback) {
  ...
  const hasExtension = hasValidImportExtension(name);
